...
def chat_with_ai(user_id: str, request: ChatRequest, session: Session = Depends(get_session)):
    ...
    try:
        ...
        try:
            ...
            ai_response_text = f"I'm sorry, I encountered an error processing your request: {str(e)}. Could you try rephrasing?"
            tool_calls_result = []
        ...
        return ChatResponse(
            ...
            tool_calls=tool_calls_result if 'tool_calls_result' in locals() else []
        )
    except Exception as e:
        # Ensure we always return the ChatResponse model even in case of errors
        return ChatResponse(
            conversation_id="",
